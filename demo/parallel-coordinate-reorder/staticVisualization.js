// global constants
globalThis.MARGIN = { top: 30, right: 10, bottom: 10, left: 30 };
globalThis.WIDTH = 960 - globalThis.MARGIN.left - globalThis.MARGIN.right;
globalThis.HEIGHT = 500 - globalThis.MARGIN.top - globalThis.MARGIN.bottom;

globalThis.parallelData = [];
globalThis.dimensions = ["Enjolras", "Eponine", "Fantine", "Gavroche", "Javert", "Marius", "Mme. Thenardier", "Myriel", "Thenardier", "Valjean"];
...
async function loadData() {
  ...
  globalThis.parallelData = globalThis.dimensions.map(d => {
    const obj = { name: d };
    globalThis.dimensions.forEach(dim => {
      if (d === dim) {
        obj[dim] = 1;
      } else {
        const link = miserables.links.find(l => 
          (miserables.nodes[l.source].nodeName === d && miserables.nodes[l.target].nodeName === dim) ||
          (miserables.nodes[l.source].nodeName === dim && miserables.nodes[l.target].nodeName === d)
        );
        obj[dim] = link ? link.value / 3 : 0;
      }
    });
    return obj;
  });
...
function renderStaticVisualization() {
  ...
  globalThis.x = d3.scalePoint()
    .range([0, globalThis.WIDTH])
    .domain(globalThis.dimensions);

  globalThis.dimensions.forEach(dimension => {
    globalThis.y[dimension] = d3.scaleLinear()
      .domain(d3.extent(globalThis.parallelData, d => d[dimension]))
