...
function renderIndexChart(root, width, height, data) {
  ...
  const linesTransformer = Libra.GraphicalTransformer.initialize(
    "linesTransformer",
    {
      layer: mainLayer,
      sharedVar: {
  ...
  const helperBarInstrument = Libra.Instrument.initialize(
    "HelperBarInstrument",
    {
  ...
  helperBarInstrument.on("hover", function ({ layer, instrument, event }) {
