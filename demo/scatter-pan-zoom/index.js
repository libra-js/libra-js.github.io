...
function renderScatterPlot (
  ...
  // mainLayer.getGraphic().setAttribute("clip-path", `url(#${clipPath.attr("id")})`);
  registerCirclesTransformer();
  const circlesTransformer = Libra.GraphicalTransformer.initialize("circlesTransformer", {
    layer: mainLayer,
    sharedVar: {
      data,
      scaleX,
  ...
  const zoomInstrument = Instrument.initialize("ZoomInstrument", {
    layers: [{ layer: mainLayer, options: { pointerEvents: 'all' } }],
    sharedVar: {
      fixRange: true,
      transformers: [circlesTransformer, axesTransformer],
    }
  });

  const panInstrument = Instrument.initialize("PanInstrument", {
    layers: [{ layer: mainLayer, options: { pointerEvents: 'all' } }],
    sharedVar: {
      fixRange: true,
      transformers: [circlesTransformer, axesTransformer],
...
function registerCirclesTransformer () {
  Libra.GraphicalTransformer.register("circlesTransformer", {
    ...
    redraw ({ layer, transformer }) {
      ...
      root.attr("clip-path", `url(#${clipPathID})`);
