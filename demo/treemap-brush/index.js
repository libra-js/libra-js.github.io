...
function renderTreemap (root, width, height, data) {
  ...
  const instrument = Libra.Instrument.initialize("BrushInstrument", {
  ...
  const transientLayer = mainLayer.getLayerFromQueue("trasientLayer");
  
  transientLayer.setLayersOrder({ "transientLayer": 2 });
...
function registerTreemapTransformer () {
  Libra.GraphicalTransformer.register("treemap", {
    ...
    redraw ({ layer, transformer }) {
      ...
      const zoomLevel = transformer.getSharedVar("zoomLevel") ?? 1;
