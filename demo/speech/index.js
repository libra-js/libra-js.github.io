...
const onClickAdd = ({ layer }) => {
  
  let [x] = layer.services.find("SelectionService").getSharedVar("x");
  let [y] = layer.services.find("SelectionService").getSharedVar("y");
...
const onClickRemove = ({ layer }) => {
  
  const results = layer.services.find("SelectionService")[0].results;
  results.then((result) => {
...
Libra.Instrument.initialize("SpeechInstrument", {
  layers: [{ layer: microPhoneLayer, options: { pointerEvents: "all" } }],
  on: {
